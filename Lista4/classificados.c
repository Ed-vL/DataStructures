#include <stdio.h>
#include <stdlib.h>
#define exch(a,b) { Semente t = b; b = a; a = t;}
...
Semente maior3(Semente a, Semente b, Semente c){
  return(a,maior2(b,c));
}
...
Semente mediana(Semente a, Semente b, Semente c){
  ...
  // if(a.nota == c.nota)
  //   return c;
  Semente m = maior3(a,b,c);
  if(m.nota == a.nota){
    return maior2(b,c);
  } else if(m.nota == b.nota){
    return maior2(a,c);
  } else {
    return maior2(a,b);
  }
...
static int separa(Semente * v, int l, int r){
  Semente c = mediana(v[l],v[(l+r)/2],v[r]);
  exch(c,v[r-1]);
  int j = l;
  ...
  for(int i = l; i < r-1; i++){
    if(v[i].nota < c.nota){
        exch(v[i],v[j]);
          j++;
       } else if (v[i].nota == c.nota && v[i].cod < c.cod){
          exch(v[i],v[j]);
          j++;
      }
  }
...
int main (){
  ...
  Semente semente;
  Semente * sementes;
  sementes = malloc(sizeof(Semente)*10000);
  scanf("%d",&k);
